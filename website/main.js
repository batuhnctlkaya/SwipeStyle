...
function askAgent() {
    ...
    .then(data => {
        ...
        if (data.question && data.options) {
            ...
            console.log("❌ Search failed - showing error page");
            hideAICreationScreen();
            if (loadingElement) loadingElement.style.display = 'none';
            showSearchFailedScreen(data.message, data.reason);
        } else if (data.error) {
            ...
            console.log("✅ Error with fallback path triggered - redirecting to search failed");
            // Instead of showing fallback recommendations, show search failed screen
            hideAICreationScreen();
            showSearchFailedScreen(
                currentLanguage === 'tr' ? 
                    'Arama sisteminde bir sorun oluştu.' : 
                    'Search system error occurred.',
                'search_error'
            );
        } else {
...
function goToHomePage() {
    ...
    console.log("🔍 Elements found:", {
        interaction: !!interactionEl,
        landing: !!landingEl
    });
    
    if (interactionEl) {
        interactionEl.style.display = 'none';
        console.log("✅ Interaction hidden");
    }
    
    if (landingEl) {
        landingEl.style.display = 'block';
        console.log("✅ Landing shown");
    }
    ...
    console.log("🏠 goToHomePage() completed");
    
    // Force reload categories to ensure clean state
    loadCategories();
}
...
function hideSearchFailedScreen() {
    const searchFailedScreen = document.getElementById('search-failed-screen');
    if (searchFailedScreen) {
        searchFailedScreen.style.display = 'none';
        console.log("✅ Search failed screen hidden");
    } else {
        console.log("⚠️ Search failed screen not found");
    }
}

function showSearchFailedScreen(message, reason) {
    // Create search failed screen if it doesn't exist
    let searchFailedScreen = document.getElementById('search-failed-screen');
    if (!searchFailedScreen) {
        searchFailedScreen = createSearchFailedScreen();
        document.body.appendChild(searchFailedScreen);
    }
    
    // Update the message
    const messageElement = searchFailedScreen.querySelector('.search-failed-message');
    const reasonElement = searchFailedScreen.querySelector('.search-failed-reason');
    
    if (messageElement) {
        messageElement.textContent = message || (currentLanguage === 'tr' ? 
    ...
    if (reasonElement && reason) {
        const reasonMessages = {
            'tr': {
                'no_serpapi_results': 'Arama servisimizde geçici bir sorun var.',
                'budget_filter_no_results': 'Seçtiğiniz bütçe aralığında ürün bulunamadı.',
                'search_engine_failed': 'Arama motoru şu anda kullanılamıyor.',
                'search_engine_error': 'Teknik bir sorun oluştu.'
            },
            'en': {
                'no_serpapi_results': 'There is a temporary issue with our search service.',
                'budget_filter_no_results': 'No products found in your selected budget range.',
                'search_engine_failed': 'Search engine is currently unavailable.',
                'search_engine_error': 'A technical issue occurred.'
            }
        };
        
        const reasonText = reasonMessages[currentLanguage]?.[reason] || reason;
        reasonElement.textContent = reasonText;
    }
    
    // Show the screen
    searchFailedScreen.style.display = 'flex';
}

function createSearchFailedScreen() {
    const screen = document.createElement('div');
    screen.id = 'search-failed-screen';
    screen.className = 'search-failed-screen';
    ...
    screen.innerHTML = `
        <div class="search-failed-content" style="text-align: center; max-width: 600px; margin: 0 auto;">
            <div class="search-failed-icon" style="font-size: 4rem; margin-bottom: 20px; opacity: 0.9;">
                <i class="fas fa-search"></i>
                <i class="fas fa-times" style="color: #fef2f2; margin-left: -20px; font-size: 2rem;"></i>
            </div>
            <h1 class="search-failed-title" style="font-size: 2.5rem; font-weight: 700; margin-bottom: 20px;">
                ${currentLanguage === 'tr' ? 'Arama Başarısız' : 'Search Failed'}
            </h1>
            <p class="search-failed-message" style="font-size: 1.2rem; margin-bottom: 15px; opacity: 0.9; line-height: 1.6;">
                ${currentLanguage === 'tr' ? 'Ürün araması yapılırken bir sorun oluştu.' : 'An error occurred while searching for products.'}
            </p>
            <p class="search-failed-reason" style="font-size: 1rem; margin-bottom: 40px; opacity: 0.8; line-height: 1.5;">
                ${currentLanguage === 'tr' ? 'Lütfen daha sonra tekrar deneyin.' : 'Please try again later.'}
            </p>
            <button class="home-btn search-failed-home-btn" onclick="goToHomePage()" style="
                ...
                margin-right: 15px;
            ">
                <i class="fas fa-home"></i> ${currentLanguage === 'tr' ? 'Ana Sayfaya Dön' : 'Go to Home Page'}
            </button>
            <button class="retry-btn search-failed-retry-btn" onclick="retrySearch()" style="
                ...
                backdrop-filter: blur(10px);
            ">
                <i class="fas fa-redo"></i> ${currentLanguage === 'tr' ? 'Tekrar Dene' : 'Try Again'}
            </button>
        </div>
    `;
    
    // Add hover effects
    const homeBtn = screen.querySelector('.search-failed-home-btn');
    const retryBtn = screen.querySelector('.search-failed-retry-btn');
    
    homeBtn.addEventListener('mouseenter', () => {
        homeBtn.style.background = 'rgba(255, 255, 255, 0.3)';
        homeBtn.style.transform = 'translateY(-2px)';
        homeBtn.style.boxShadow = '0 8px 24px rgba(0, 0, 0, 0.2)';
    });
    ...
    retryBtn.addEventListener('mouseenter', () => {
        ...
        retryBtn.style.boxShadow = '0 8px 24px rgba(0, 0, 0, 0.2)';
    });
...
function retrySearch() {
    // Hide search failed screen
    const searchFailedScreen = document.getElementById('search-failed-screen');
    if (searchFailedScreen) {
        searchFailedScreen.style.display = 'none';
    }
    
    // Go back to interaction area or home
    goToHomePage();
}
