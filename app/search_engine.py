...
class ModernSearchEngine:
    ...
    def search_products(self, user_preferences: Dict, site_filter: Optional[List[str]] = None) -> Dict:
        ...
        try:
            ...
            # Check if we have any real recommendations
            if not final_recommendations:
                print(f"❌ No real recommendations available - search failed")
                return {
                    'status': 'no_results',
                    'message': 'Maalesef bu kriterlere uygun ürün bulunamadı. Arama servisimizde geçici bir sorun olabilir.',
                    'reason': 'no_serpapi_results',
...
"""
    ...
    def _search_shopping_serp(self, preferences: Dict) -> List[Dict]:
        ...
        """
        if not self.serpapi_key:
            print("⚠️ SERPAPI_KEY bulunamadı - gerçek arama yapılamıyor")
            return []  # Return empty list instead of mock results
    ...
    def _generate_structured_recommendations(self, grounding: Dict, shopping: List[Dict], preferences: Dict) -> List[Dict]:
        ...
        try:
            ...
            # ✅ Fallback: SerpAPI sonucu yoksa boş liste döndür (mock kullanma)
            else:
                print(f"⚠️ No SerpAPI results available - returning empty list")
                return []  # Return empty list instead of mock recommendations
