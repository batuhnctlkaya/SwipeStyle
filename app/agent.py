...
class Agent:
    ...
    def _generate_recommendations(self, category, preferences, specs, language):
        ...
        try:
            ...
            if search_results['status'] == 'no_results':
                print(f"❌ Search failed completely - no real results available")
                return {
                    'type': 'search_failed',
                    'message': search_results.get('message', 'Ürün araması başarısız oldu'),
                    'reason': search_results.get('reason', 'unknown'),
                    'category': category,
            ...
            if search_results['status'] == 'success' and search_results.get('recommendations'):
                ...
                # Eğer budget filtreleme sonrası hiç ürün yoksa search failed döndür
                if not filtered_recommendations:
                    print(f"⚠️ Budget filtreleme sonrası hiç ürün kalmadı")
                    return {
                        'type': 'search_failed',
                        'message': f'Seçtiğiniz bütçe aralığında ({preferences.get("budget_band", "belirsiz")}) ürün bulunamadı. Lütfen bütçe aralığınızı genişletmeyi deneyin.',
                        'reason': 'budget_filter_no_results',
                        'category': category,
                        'preferences': preferences,
                        'confidence_score': self._calculate_confidence_score(preferences, specs),
                ...
                print(f"⚠️ Modern search engine başarısız veya boş sonuç")
                return {
                    'type': 'search_failed',
                    'message': 'Arama sistemi geçici olarak kullanılamıyor veya kriterlerinize uygun ürün bulunamadı. Lütfen daha sonra tekrar deneyin.',
                    'reason': 'search_engine_failed',
                    'category': category,
                    'preferences': preferences,
                    'confidence_score': self._calculate_confidence_score(preferences, specs)
                }
            ...
            return {
                'type': 'search_failed',
                'message': 'Arama sistemi geçici olarak kullanılamıyor. Lütfen daha sonra tekrar deneyin.',
                'reason': 'search_engine_error',
                'error_details': str(e),
                'category': category,
                'preferences': preferences,
                'confidence_score': self._calculate_confidence_score(preferences, specs)
